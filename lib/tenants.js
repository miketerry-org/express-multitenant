...
class Tenants {
  ...
  #dbConnectFunc; // Database connection function
   ...
   *
   * @param {Function} dbConnectFunc - A function to connect to the dbConfig. It should accept a config parameter.
   ...
   */
  constructor(dbConnectFunc, rule = undefined) {
    this.#list = [];
    this.#topsecret = new TopSecret();

    // Assign database connection function
    this.#dbConnectFunc = dbConnectFunc;
    if (!this.#dbConnectFunc || typeof this.#dbConnectFunc !== "function") {
      throw new Error(
        `"dbConnectFunc" is required and must be a function with a single "config" parameter`
      );
    }
  ...
  add(data) {
    ...
    const tenant = new Tenant(data, this.#dbConnectFunc);
